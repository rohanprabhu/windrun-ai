...
There is no Shared VPC and no runtime network connection between production and staging. Each load balancer, backend service, serverless NEG, certificate, image repository, runtime service account, and Cloud Run service lives in the same project as the environment it serves. Explicit `gcp.Provider` instances are required for all resources and default GCP providers are disabled, preventing ambient CLI configuration from placing a resource in the wrong project. Only the foundation stack can write to the shared Cloud DNS zone. Routine application deployment identities cannot alter DNS, certificates, project IAM, billing, or resources in another project. The manually approved foundation identity is the sole cross-project control-plane identity; its Project IAM Admin permission is condition-limited to the exact roles this Pulumi program manages and cannot modify its own limiting grant, Owner, Editor, or unrelated roles. It has no organization or billing-account role.
...
| Stack | Owns |
| --- | --- |
| `foundation` | Three GCP projects, required APIs, Artifact Registry repositories, reserved global IP addresses, Cloud DNS zone, DigitalOcean NS delegation, DNS A/wildcard records, Certificate Manager DNS authorizations/certificates/maps, runtime service accounts, project-local IAM, and GitHub/GCP federation |
| `delivery` | Pulumi Cloud GitHub OIDC issuer plus GitHub deployment environments, deployment-branch policies, and repository Actions variables; this post-claim stack has no GCP resources |
...
| Git event | Action |
| --- | --- |
| Pull request to `main` opened, reopened, or synchronized | Test, build, deploy/update stack `pr-<number>` in the staging project, smoke-test its wildcard URL, and comment/update the preview URL |
| Pull request to `main` closed | Destroy `pr-<number>`, then remove the empty Pulumi stack |
...
Pulumi Cloud trusts GitHub OIDC tokens only for immutable repository ID `1095528250`, owner ID `136263`, and repository `rohanprabhu/windrun-ai`. Trust does not depend on one literal GitHub `sub` prefix: GitHub's immutable owner/repository-ID subject format becomes the default for newly created repositories on 2026-07-15 and can later apply after rename/transfer. Pulumi policies therefore combine numeric claims with an environment suffix pattern, and GCP maps `google.subject` to the immutable repository ID rather than the mutable name-based subject. GCP uses separate Workload Identity pools/providers and service accounts for production, staging, preview, production edge, staging edge, and foundation operations; mappings include only claims guaranteed for that provider, with `job_workflow_ref` added only to preview trust. Production trust accepts only the `main` branch workflow. Staging trust accepts only the `staging` branch workflow. Preview trust accepts same-repository pull-request workflows and receives Cloud Run, Artifact Registry write, and runtime-service-account impersonation permissions only in the staging project. Cloud Run administrative permissions cannot be IAM-conditioned by service-name prefix, so credentialed preview jobs run the Pulumi program from a separate trusted `main` checkout on a fresh runner; PR code is supplied only as the Docker build context, and the trusted program derives the exact `pr-<number>` service name. The reusable workflows are pinned to `main`, and their identity conditions match the exact reusable-workflow reference. Pull requests from forks run uncredentialed tests only and cannot deploy. Workflows never use `pull_request_target` to execute pull-request code. No CI identity receives a static Google service-account key.

The Pulumi account is currently an unclaimed agent account. Code and the local GCP bootstrap deployment may proceed during its 72-hour write window, but the `delivery` stack is created only after the user claims the account with `rohan@windrun.ai` and the agent authenticates to the transferred account. Claiming invalidates the ephemeral credential, so this is an explicit final-bootstrap checkpoint rather than an implicit background step. The delivery stack uses the local GitHub login only as provider authentication; all repository mutations remain Pulumi resources. Initial project creation, any project metadata/billing repair, and final project deletion remain local Pulumi operations guarded by exact active-account and ADC checks for `rohan@windrun.ai`; no CI identity receives organization or billing-account roles.
...
5. Verify active gcloud and ADC identities are exactly `rohan@windrun.ai`, set foundation config `allowProjectDeletion=true`, preview the resulting unprotection/deletion-policy-only change, and apply it with `pulumi up`. Verify `protect:false` and project `deletionPolicy: DELETE` in state, then destroy `foundation`. It removes delegated DNS records, Cloud DNS, certificates, reserved IPs, registries, IAM/federation, and finally schedules all three GCP projects for deletion. If destruction stops partway, reset the flag and apply foundation again to re-protect every survivor.

Every foundation resource is protected while `allowProjectDeletion=false`; project resources also use deletion policy `PREVENT`. Both protections and `DELETE` are persisted only by the acknowledged local teardown update. The foundation destroy script requires an explicit `--destroy-projects` acknowledgement and verifies that no application, preview, or edge stacks still contain resources. Project deletion is the final recovery boundary; deleted projects enter Google Cloud `DELETE_REQUESTED` state and retain a 30-day recovery window, while their project IDs remain permanently unavailable for reuse.
